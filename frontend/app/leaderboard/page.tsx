...
export default async function LeaderboardPage() {
  ...
  const leaderboardData = await fetch("http://localhost:8080/api/leaderboard", {
    cache: "no-store",
  })
    .then((res) => res.json())
    .catch(() => [])
            ...
            <Card className="order-1 p-6 md:order-1">
              <div className="flex flex-col items-center text-center">
                ...
                <Avatar className="mb-3 h-16 w-16">
                  ...
                  />
                  <AvatarFallback className="bg-primary/10">{leaderboardData[1]?.username?.[0]}</AvatarFallback>
                </Avatar>
            ...
            <Card className="order-2 border-primary bg-primary/5 p-6 md:order-2">
              <div className="flex flex-col items-center text-center">
                ...
                <Avatar className="mb-3 h-20 w-20 ring-2 ring-primary">
                  ...
                  />
                  <AvatarFallback className="bg-primary/10">{leaderboardData[0]?.username?.[0]}</AvatarFallback>
                </Avatar>
            ...
            <Card className="order-3 p-6 md:order-3">
              <div className="flex flex-col items-center text-center">
                ...
                <Avatar className="mb-3 h-16 w-16">
                  ...
                  />
                  <AvatarFallback className="bg-primary/10">{leaderboardData[2]?.username?.[0]}</AvatarFallback>
                </Avatar>
            ...
            <TableBody>
              {leaderboardData.map((player: any, index: number) => (
                <TableRow key={player.userId}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <Avatar className="h-8 w-8">
                        ...
                        <AvatarFallback className="bg-primary/10 text-xs">{player.username?.[0]}</AvatarFallback>
                      </Avatar>
                  ...
                  <TableCell className="text-right">{((player.wins / player.gamesPlayed) * 100).toFixed(1)}%</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
