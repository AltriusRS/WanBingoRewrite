...
--
-- Notes
-- - Handlers already query table `tiles (text, category, weight, last_drawn_show)`.
...
CREATE TABLE IF NOT EXISTS tiles (
    ...
    last_drawn_show   INTEGER,                    -- loosely used marker; app may treat as show index
...
CREATE TABLE IF NOT EXISTS shows (
    id            BIGSERIAL PRIMARY KEY,
    external_id   TEXT UNIQUE,               -- e.g., a WhenPlane identifier if/when one exists
...
CREATE TABLE IF NOT EXISTS tile_confirmations (
    ...
    confirmed_by  TEXT,                     -- free-form identifier for host/operator
...
-- 7) Seed helper (optional): copy JSON tiles into DB
-- You can import data/data/tiles.json externally or with a small script.
-- Example (psql + jq):
-- jq -c '.[]' data/tiles.json | while read -r row; do
--   psql "$DATABASE_URL" -c "
--     INSERT INTO tiles(text, category, weight, last_drawn_show)
--     VALUES (
--       $(echo "$row" | jq -r '.text | to_json'),
--       $(echo "$row" | jq -r '.category | to_json'),
--       $(echo "$row" | jq -r '.weight'),
--       $(echo "$row" | jq -r '.last_drawn_show // 0')
--     ) ON CONFLICT (text) DO UPDATE SET
--       category = EXCLUDED.category,
--       weight = EXCLUDED.weight,
--       last_drawn_show = EXCLUDED.last_drawn_show;"; done;
